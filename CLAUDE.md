...
Read `docs/decisions.md`. It records why the app is shaped as it is, including choices that look wrong until you know the reason. Do not silently revert a logged decision — raise it.

For status on the review-harness side project (the `review/` folder, the live Vercel deployment, what's built vs. not), read `review-build.md` first — it's the up-to-date log, not this file.
...
- **Never edit anything under `prototypes/`.** Not one character — it's the historical record of what v2 is. Copy content out into `review/` or elsewhere; never edit in place.
- **Mock data lives in the logic class**, mirrored to `data/postings.json`. Change both or neither.
